...
namespace TikStudio::Tests
{
    ...
    {
        Tests.push_back({"Complete Chat conversion", &TestCompleteConversion});
        Tests.push_back({"Optional field defaults", &TestOptionalFieldDefaults});
        Tests.push_back({"Non-Chat event is ignored", &TestNonChatEventIsIgnoredWithoutChatInspection});
        Tests.push_back({"Invalid envelope and missing data or user", &TestInvalidEnvelopeAndMissingDataOrUser});
        Tests.push_back({"Missing or empty user identity", &TestMissingOrEmptyUserIdentity});
        Tests.push_back({"Emote-only Chat is converted", &TestEmoteOnlyChatIsConverted});
        Tests.push_back({"Empty content is rejected", &TestEmptyContentIsRejected});
        Tests.push_back({"Invalid emotes reject whole message", &TestInvalidEmoteVariantsRejectWholeMessage});
        Tests.push_back({"Numeric representation boundaries", &TestNumericRepresentationBoundaries});
        Tests.push_back({"Text and duplicate emotes are preserved", &TestTextAndDuplicateEmotesArePreserved});
    }
}
