...
int main()
{
    const std::vector<FTestCase> Tests{
        {"Empty Host cycle", &TestEmptyHostCycle},
        {"Worker PostChat runs on owner", &TestWorkerPostRunsOnOwner},
        {"Sequential FIFO and asynchronous completion", &TestSequentialFifoAndAsynchronousCompletion},
        {"Worker completion releases capacity", &TestWorkerCompletionReleasesCapacity},
        {"Cancel advances with explicit Pump", &TestCancelAdvancesWithExplicitPump},
        {"Wrong completion preserves later command", &TestWrongCompletionPreservesLaterCommand},
        {"Pending Chat expires while another is Processing", &TestPendingChatExpiresWhileAnotherIsProcessing},
        {"RunOneCycle rejects wrong thread before work", &TestRunOneCycleRejectsWrongThreadBeforeWork},
        {"Explicit Pump works when Auto Pump is disabled", &TestExplicitPumpWorksWhenAutoPumpIsDisabled}
    };
